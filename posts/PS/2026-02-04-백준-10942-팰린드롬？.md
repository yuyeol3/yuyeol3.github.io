...
그러나 또 하나의 고민은 점화식을 어떤 순서로 계산해야 하는가이다. 이를 탐구하기 위해 아래 표를 보자. 아래 표는 `dp[s][e]`를 계산할 때 참조해야 하는 이전 (s,e)를 작성해 놓은 표이다. 예를 들어 `dp[2][5]`를 알아내고 싶다면 `dp[3][4]`의 값을 참조해야 한다. 또한 x표시는 이전 dp값을 참조할 필요가 없다는 뜻이다. 문자열의 길이가 1개이거나 2개이면 내부에 문자열이 더 없기 때문이다.
