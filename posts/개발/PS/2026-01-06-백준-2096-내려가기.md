...
title: 2096 - 내려가기
description: >-
  이 문제는 Nx3칸의 테이블에서 규칙을 따라 내려가면서 얻을 수 있는 최대 점수 및 최소 점수를 구해야 하는 문제이다. 규칙은 다음과 같다.
  어떤 줄에서 다음 줄로 넘어갈 때 바로 아래의 수로 내려가거나, 또는 바로 아래의 수와 붙어 있는 수로만 이동할 수 있다.
---
...
그런 다음 기저 조건을 설정하자. 첫 번째 행에 있는 칸을 방문하면 그 상태에서 최대, 최소 점수는 방문한 그 칸의 점수뿐이므로
